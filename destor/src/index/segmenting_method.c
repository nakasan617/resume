...
static struct segment* segment_fixed(struct chunk * c) {
    ...
    printf("this is chunk_num %ld and destor.index_segment_algorithm[1] %ld\n", tmp->chunk_num, destor.index_segment_algorithm[1]);
...
static struct segment* segment_content_defined(struct chunk *c) {
    ...
    g_sequence_append(tmp->chunks, c);
    tmp->chunk_num++;
    //printf("this is destor.index_segment_max %d\n", destor.index_segment_max);
