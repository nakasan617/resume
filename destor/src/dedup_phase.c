...
void *dedup_thread(void *arg) {
	printf("came to dedup thread\n");
	struct segment* s = NULL;
	while (1) {
		struct chunk *c = NULL;
		if (destor.simulation_level != SIMULATION_ALL)
			c = sync_queue_pop(POS_queue);
		else
			c = sync_queue_pop(trace_queue);

		/* Add the chunk to the segment. */
		//printf("I am about to do the segmenting\n");
		s = segmenting(c);
		if (!s)
			continue;
		/* segmenting success */
...
void start_dedup_phase() {
	...
	printf("this is before calling dedup_thread\n");
	pthread_create(&dedup_t, NULL, dedup_thread, NULL);
	printf("you can print this out correct?\n");
}
