...
static void* chunk_thread(void *arg) {
	...
	while (1) {
		...
		while (1) {
			// printf("this is the %d th time we are chunking\n", count++);
			/* c == NULL indicates more data for this file can be read. */
			while ((leftlen < destor.chunk_max_size) && c == NULL) {
				c = sync_queue_pop(read_queue);
				// if it is not at the end you keep on reading and adding to leftbuf
				if (!CHECK_CHUNK(c, CHUNK_FILE_END)) {
			...
			// if they are the same then just add it to the zero byte thingy
			if (memcmp(zeros, nc->data, chunk_size) == 0) {
				VERBOSE("Chunk phase: %ldth chunk  of %d zero bytes",
						chunk_num++, chunk_size);
				// but they are zero bytes
				jcr.zero_chunk_num++;
				jcr.zero_chunk_size += chunk_size;

			// if they are different, most of them come here
			} else
...
void start_chunk_phase() {
	...
	chunk_queue = sync_queue_new(128);
	// I changed this from 100 to 128
