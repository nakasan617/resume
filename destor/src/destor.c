...
/* my ad-hoc solution hopefully it works */
extern struct POS_tree;
extern struct POS_tree * ptree;
...
void tokenize_arg(int * inc, char ** arr) {
	printf("arr is at: %x\n", arr);
	int count = 1;
	char line [256];
	strncpy(arr[0],"destor", 10); /* made it strncpy just in case */ 
	if(fgets(line, sizeof(line), stdin)) {
		for(arr[count] = strtok(line, " \n"); arr[count] != NULL; arr[count] = strtok(NULL, " \n"))
			count++;
	}
	printf("count: %d\n", count);
	*inc = count;
...
int main(int argc, char **argv) {
	...
	while (1) {
		...
		int revision = -1;
		int opt = 0;
		int job = DESTOR_UPDATE;
		if(ptree == NULL) {
			job = DESTOR_BACKUP;
		}
		if(!strcmp(arr[1], "quit")) {
			printf("came here\n");
			break;
		}
 
		while ((opt = getopt_long(inc, arr, short_options, long_options, NULL))
				!= -1) {
			switch (opt) {
			case 'r':
				job = DESTOR_RESTORE;
                printf("this is optarg: %s\n", optarg);
                assert(optarg != NULL);
		...
		sds path = NULL; // sds is just CHAR *

		switch (job) {
		case DESTOR_BACKUP:

			if (inc > optind) {
				path = sdsnew(arr[optind]);
			} else {
				fprintf(stderr, "backup job needs a protected path!\n");
				usage();
			}

			printf("backup with path %s\n", path);
			do_backup(path);
		...
		case DESTOR_UPDATE: {
			/* 
			  What information do I need to update??
				1. Which chunks were updated. -> int *
				2. Updated chunks. -> char **
			  
			   since it is hard to give ones with command line arguments, 
			   just write to a file and read it from the file.
			*/
				
			if (inc > optind) {
				path = sdsnew(arr[optind]);
			} else {
				fprintf(stderr, "update needs a protected path!\n");
				usage();
			}
			// do_update(path);
			break;
